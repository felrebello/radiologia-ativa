...
type Props = {
  ...
  onClose: () => void;
  lesson: Lesson;
  onSave: (lessonId: string, payload: Partial<Lesson>) => void;
};
...
function toDateInputValue(input: any) {
  const d = input instanceof Date ? input : (typeof input?.toDate === 'function' ? input.toDate() : new Date(input));
  if (isNaN(d as any)) return '';
  return d.toISOString().slice(0,10);
}
function toTimeInputValue(input: any) {
  const d = input instanceof Date ? input : (typeof input?.toDate === 'function' ? input.toDate() : new Date(input));
  if (isNaN(d as any)) return '';
  return d.toTimeString().slice(0,5);
}

export default function EditLessonModal({ isOpen, onClose, lesson, onSave }: Props) {
  const [title, setTitle] = React.useState(lesson.title || '');
  const [description, setDescription] = React.useState(lesson.description || '');
  const [date, setDate] = React.useState<string>(toDateInputValue(lesson.date));
  const [time, setTime] = React.useState<string>(toTimeInputValue(lesson.date));
  const [duration, setDuration] = React.useState<number>(lesson.duration || 60);
  const [materials, setMaterials] = React.useState<Material[]>(lesson.materials || []);
  ...
  function handleSave() {
    const dt = (date && time) ? new Date(`${date}T${time}`) : lesson.date;
    onSave(lesson.id, { title, description, date: dt, duration, materials });
    onClose();
  }
    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl">
        <div className="flex items-center justify-between p-4 border-b">
          <h3 className="text-lg font-semibold">Editar Aula</h3>
        ...
        <div className="p-4 border-t flex justify-end gap-2">
          ...
          <button onClick={handleSave} className="px-3 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white">Salvar</button>
        </div>
      </div>
    </div>
  );
